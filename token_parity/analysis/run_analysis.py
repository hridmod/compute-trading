"""Run the token-parity ceiling test on a fresh snapshot, once per pricing basket.
...
from src.pull_llm_pricing import MODEL_BASKETS, pull_llm_pricing  # noqa: E402
...
def main():
    ...
    all_results = []
    for basket in MODEL_BASKETS:
        results = model.compute_ceiling_test(mlperf_rows, rental_rows, llm_pricing_rows, basket)
        all_results.extend(results)

        print(f"\ntoken-parity ceiling test -- {today} -- basket: {basket}")
        if not results:
            print("  no results (basket had no priced models, or no rental/benchmark overlap)")
            continue
        print(f"market rate (median completion $/token): {results[0]['market_usd_per_token']:.8f}")
