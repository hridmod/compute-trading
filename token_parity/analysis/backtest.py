"""Backtest the token-parity ceiling across every accumulated snapshot in
data/history/ceiling_snapshots.csv.

Each run of run_analysis.py appends one row per GPU for that day. Run this
script on a cadence (see .github/workflows/daily_snapshot.yml) to build up
history, then use this script to see whether the ceiling has ever bound,
and how the rental/ceiling ratio trends over time.

With only one accumulated snapshot, this necessarily reduces to the
cross-sectional test in run_analysis.py -- that's an honest null result,
not a bug, and is called out in README.md.
"""
import sys
